...
//Rare exceptions to indicate a bug in the _cohistoria code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_cohistoria = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_cohistoria]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.cohistoria.module"
